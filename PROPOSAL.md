...
Use Python (pandas, NumPy, scikit-learn) to preprocess and analyze a dataset of professional rugby players from Kaggle (e.g., Six Nations or Rugby Championship statistics spanning 3-5 seasons). Create predictive features: past performance averages (rolling windows), minutes played, age, position, and workload metrics. Train and compare regression models — Linear Regression, Random Forest, and Gradient Boosting — to predict a player's future performance metric (e.g., points or overall rating per match). Models will be trained using time-based cross-validation to respect temporal data structure. Evaluate models using MAE, RMSE, and R², comparing against a simple baseline (e.g., average of recent performances).
...
- Exploring hyperparameter tuning or ensemble averaging to improve prediction accuracy.